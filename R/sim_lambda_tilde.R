...
sim_lambda_tilde <- function(T, n1, gamma = 0.98, a0 = 20, b0 = 20,
  ...
  if (!is.null(cp_time) && cp_time > 0 && cp_time < T) {
    lam[(cp_time + 1):T, ] <- lam[(cp_time + 1):T, , drop = FALSE] * cp_strength
  }
