...
#
# Key change vs sampler_algo2.R:
# - Update discounts (gamma, delta) BEFORE FFBS draws for (lambda_tilde, omega),
#   so the latent trajectories are sampled conditional on the most recent discounts.
# - Uses a deterministic xi = e * kappa * exp(eta) for the gamma grid update,
#   computed from the current regression state (beta0, beta, phi) and augmentation (kappa).
# - 4-space indentation.
...
compute_xi_gamma <- function(e1, x1, beta0, beta, phi, kappa, Xmat = NULL) {
    # Deterministic exposure for the Gamma-Poisson (lambda_tilde) filter:
    #   y_{t,j} | lambda_{t,j} ~ Poisson( xi_{t,j} * lambda_{t,j} )
    # where xi_{t,j} = e_{t,j} * kappa_{t,j} * exp(eta_{t,j})
    # and eta_{t,j} = beta0 + x_{t,j}^T beta + phi_j.
    TT <- nrow(e1)
    n1 <- ncol(e1)
    p <- dim(x1)[3]

    if (!exists("clamp", mode = "function")) {
        clamp <- function(z, lo = -30, hi = 30) {
            pmin(pmax(z, lo), hi)
        }
    }
...
msstnb_algo2_mcmc <- function(data, hyper, mcmc,
    ...
    for (iter in seq_len(n_iter)) {
        ...
        # 9) Update discounts gamma and delta (Section 8.6) BEFORE FFBS
        if (update_discounts_every > 0 && (iter %% update_discounts_every == 0)) {

            # gamma update needs xi = e * kappa * exp(eta)
            xi_gamma <- compute_xi_gamma(
                e1 = e1,
                x1 = x1,
                beta0 = state$beta0,
                beta = state$beta,
        ...
        if (iter > burn && ((iter - burn) %% thin == 0)) {
            ...
            if (store_phi) {
                phi_draw[save_idx, ] <- state$phi
            }
            if (store_lambda) {
                lambda_draw[save_idx, , ] <- state$lambda_tilde_0T
            }
            if (store_gamma) {
                gamma_draw[save_idx, ] <- state$gamma
            }
            if (store_r) {
                r_draw[save_idx, ] <- state$r1
            }
            if (store_delta) {
        ...
        if (verbose && (iter %% max(1L, floor(n_iter / 10)) == 0)) {
            cat(sprintf(
                "Iter %d/%d | acc_r=%.2f | ESS(beta)=%.0f | ESS(phi)=%.0f\n",
                iter,
                n_iter,
                mean(acc_r[pmax(1, iter - 99):iter]),
