# update_beta.R
# Faster ESS update for (beta0, beta).
#
# Speed improvements:
# - Vectorized likelihood (no loop over regions)
...
prep_beta_prior <- function(p, m0 = 0, s0 = 10, m_beta = NULL, V_beta = NULL) {
...
prep_beta_ess <- function(y1, e1, kappa, lambda_tilde_0T, phi, Xmat = NULL,
    ...
    if (is.null(Xmat)) {
        ...
        p <- dim(x1)[3]
        if (p > 0) Xmat <- prep_Xmat(x1) else Xmat <- NULL
    } else {
        p <- ncol(Xmat)
    }

    list(TT = TT, n1 = n1, p = p,
         y_vec = y_vec,
...
update_beta_ess_fast <- function(beta0, beta, prep, prior) {
    ...
    # draw_nu <- function() {
    #   as.numeric(t(prior$cholSigma) %*% rnorm(prior$k))
    # }
    draw_nu <- function() {
...
# Backward compatible wrapper (same signature as the original implementation).
# Optional speed args:
# - Xmat: cached (TT*n1 x p) design matrix
# - prior_prep: cached output of prep_beta_prior()
update_beta_ess <- function(beta0, beta, phi, lambda_tilde_0T, kappa, y1, e1, x1,
    ...
    if (is.null(prior_prep)) {
        prior_prep <- prep_beta_prior(p = p, m0 = m0, s0 = s0, m_beta = m_beta,
                                      V_beta = V_beta)
    }

    prep <- prep_beta_ess(y1 = y1, e1 = e1, kappa = kappa,
                          lambda_tilde_0T = lambda_tilde_0T,
                          phi = phi, Xmat = Xmat, x1 = x1)
