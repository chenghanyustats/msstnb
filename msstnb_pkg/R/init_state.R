...
init_state <- function(data, hyper, seed = NULL, cache = NULL) {
    set_seed(seed)
    ...
    lambda_tilde_0T <- matrix(1, nrow = TT + 1L, ncol = n1)
    ...
    for (l in seq_len(L - 1L)) {
        ...
        for (j in seq_len(n_parent)) {
            ...
            om <- matrix(NA_real_, nrow = TT + 1L, ncol = K)
